(function (root, factory) {
    ...
    function mapMouseEventToAction(type) {
        if (type === 'click') return 'SELECT_CARD';
        return null;
    }
    ...
    function mapKeyEventToAction(event) {
        ...
        if (event.code === 'Escape' && event.type === 'keydown') return 'CANCEL';
    ...
    function setModeText(mode) {
        ...
        if (root.document) {
            root.document.querySelectorAll('[data-control-mode]').forEach(button => {
                button.classList.toggle('active', button.dataset.controlMode === mode);
            });
        }
        if (mode === 'mouse') {
            if (status) status.innerText = '鼠标模式 / Mouse Mode';
    ...
    function isInteractiveElement(target) {
        return Boolean(target && target.closest && target.closest('button, a, input, select, textarea, label, .stage-panel, .template-ring, .control-chooser'));
    }
    ...
    function startMouseMode() {
        ...
        root.window.addEventListener('click', event => {
            if (typeof activeInputMode !== 'undefined' && activeInputMode !== 'mouse') return;
        ...
        root.window.addEventListener('keydown', event => {
            ...
            if ((action === 'CONFIRM' || action === 'CANCEL') && typeof root.handleMouseKeyboardAction === 'function') {
                root.handleMouseKeyboardAction(action);
                return;
            }
